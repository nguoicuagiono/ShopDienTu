...
//	Đếm tất cả các hóa đơn đến ngày hiện tại
	@Query("SELECT count(*) FROM Order o WHERE orderDate = :date")
	long countItemByDate(@Param("date") Date date) ;
	
//	Tính tổng doanh thu của shop
	@Query("select sum(amount) from Order")
	long sumTotalRevenue();
	
//	Tính tổng doanh thu ở tháng hiện tại
	@Query("select sum(p.amount) from Order p where p.orderDate between :fromDate and :toDate")
	long TotalSalesForTheCurrentMonth(@Param("fromDate") Date firstDate, @Param("toDate") Date lasttDate);
